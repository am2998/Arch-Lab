...
pacstrap /mnt linux base base-devel linux-firmware lvm2 zram-generator reflector man sudo nano git networkmanager amd-ucode
...
sed -i '1s/^/title Arch Linux\n/' /boot/loader/entries/arch.conf
sed -i '2s/^/linux \/vmlinuz-linux\n/' /boot/loader/entries/arch.conf
sed -i '3s/^/initrd \/amd-ucode.img\n/' /boot/loader/entries/arch.conf
sed -i '4s/^/initrd \/initramfs-linux.img\n/' /boot/loader/entries/arch.conf
UUID_VALUE=$(blkid -s UUID -o value ${DISK}${PARTITION_2})

sed -i "5s|^|options cryptdevice=UUID=$UUID_VALUE:cryptroot root=/dev/mapper/sys-root rootfstype=btrfs rootflags=subvol=@\n|" /boot/loader/entries/arch.conf

echo ' ' > /boot/loader/loader.conf
echo -e "default arch\ntimeout 0\nconsole-mode max" | sudo tee -a /boot/loader/loader.conf > /dev/null
...
pacman -S --noconfirm net-tools flatpak firefox konsole dolphin okular kate 
